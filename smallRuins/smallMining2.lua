return function(center, surface) -- small mining
    ...
    ce{name="electric-mining-drill", position={center.x + (1.0), center.y + (2.0)}, direction = direct.north, force = fN}
    ...
    local e = ce{name="transport-belt", position={center.x + (1.0), center.y + (0.0)}, direction = direct.west, force = fN}
    if e then
      e.damage(15,"neutral","physical")
    end
    local e = ce{name="transport-belt", position={center.x + (2.0), center.y + (0.0)}, direction = direct.west, force = fN}
    if e then
